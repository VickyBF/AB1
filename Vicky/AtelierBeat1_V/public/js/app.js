...
//// check where you are and after confront the input with name of schema depending on your position
...
//
//        doJSONRequest('GET','/tracks', null, null, function(tracksList){
//            var tracksFounded = fuzzyFind(tracksList, "name", value);
//            result['tracks'] = tracksFounded;
//        });
//
//        doJSONRequest('GET','/artists', null, null, function(artistsList){
//            var artistsFounded = fuzzyFind(artistsList, "name", value);
//            result['artists'] = artistsFounded;
//        });
//
//        doJSONRequest('GET','/albums', null, null, function(albumsList){
//            var albumsFounded = fuzzyFind(albumsList, "name", value);
//            result['albums'] = albumsFounded;
//        });
//        console.log(result);
//
//    });
...
function previewResults(evt, query) {
    // This function was meant to filter keyCodes like Shift etc., but I think this implementation is better
    if (query != lastSearch) {
        previewR(query);
...
function search(from, query) {
    ...
    doJSONRequest('GET', '/tracks', {}, null, function (obj) {
        ...
        if (result.length == 0)
            tracksCont = '<h2>No tracks found</h2>';
            viewFoundedResult();
    });
    ...
    doJSONRequest('GET', '/albums', {}, null, function (obj) {
        ...
        if (result.length == 0)
            albumsCont = '<h2>No albums with such name</h2>';
            viewFoundedResult();
    });
    ...
    doJSONRequest('GET', '/artists', {}, null, function (obj) {
        ...
        if (result.length == 0)
            artistsCont = '<h2>No artists found</h2>';
            viewFoundedResult();
    });
}
