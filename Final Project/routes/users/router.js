...
router.get('/', function(req, res, next) {
  ...
  User.find({}, fieldsFilter).lean().populate("myVotedSongs").exec(function(err, users){
...
router.get('/:userid', function(req, res, next) {
  User.findById(req.params.userid, fieldsFilter).lean().populate("myVotedSongs").exec(function(err, user){
    if (err) return next (err);
    if (!user) {
      res.status(404);
      res.json({
